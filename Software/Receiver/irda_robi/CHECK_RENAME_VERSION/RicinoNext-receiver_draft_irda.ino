...
#define RX 8 //PIN_PB1 //8; // PB1
#define TX 9 // PIN_PB0 //9; // PB0
...
//  static uint16_t loopTimer = millis();
  ...
  if(Serial.available() && !messagePending && stateRace) {
  ...
  if ( messagePending){
      if (idInfo[2] == 0x84){ // car pass
          ...
          SoftSerialDebug.print(idInfo[11], DEC);
          SoftSerialDebug.print(" | ");
          SoftSerialDebug.print("strength : ");
          SoftSerialDebug.print(idInfo[12], DEC);
...
//      }
  ...
  // read from port 0, send to port 1:
    if (SoftSerialDebug.available()) {
    
        byte inByte = SoftSerialDebug.read();
...
//    digitalWrite(LED_PIN, ledState);
    counter++;
    SoftSerialDebug.println(counter);
  }
}
